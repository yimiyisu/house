package com.yimiyisu.house.hooks.commerceArea;
...
public class DeleteHook implements IHook {
    ...
    public ZenResult before(ZenData data) {
        ZenResult houses = zenEngine.execute("count/commerce_area", data);
        if (houses.getLong() > 0) {
            return ZenResult.fail("有关联区域不可删除");
        }
        ZenResult rooms = zenEngine.execute("count/house", data);
        if (rooms.getLong() > 0) {
            return ZenResult.fail("有关联商铺不可删除");
        }
