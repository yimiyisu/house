<template>
    <z-page-title value="车位规划" />
    <el-row :gutter="20">
        <el-col :span="6">
            <z-action label="添加区域" url="/do/put/parking_area" :fields="['title', 'isLeaf']" @finish="(result) => addFinish(result, node)" />
                    ...
                    <span class="custom-tree-node">
                        ...
                        <div class="actions">
                            <z-action icon="plus" type="text" v-if="data.isLeaf == 0" :title="'添加' + data.title + '子区域'"
                                url="/do/put/parking_area" :data="{ parentId: data.id }" :fields="['title', 'isLeaf']"
                                @finish="(result) => addFinish(result, node)" />
                            <z-action icon="edit" type="text" :title="'修改' + data.title + '子区域'" url="/do/patch/parking_area"
                                :data="data" :fields="['title']" @finish="(result) => edit(result, node)" />
                            <z-action icon="x" type="text" url="/do/delete/parking_area" mode="confirm" :data="data"
        ...
        <el-col :span="18">
            <z-block v-if="params" href="/setting/parking" :params="params" />
        </el-col>
    </el-row>
</template>
...
export default {
    ...
    methods: {
        ...
        async loadNode(node, callback) {
            const result = await $.get({ url: "/do/list/parking_area", data: { parentId: node.data.id } })
            callback(result)
        },
